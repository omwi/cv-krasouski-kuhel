...
function FieldTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="field-label"
      className={cn(
        "gap-2 text-sm font-medium flex w-fit items-center group-data-[disabled=true]/field:opacity-50",
        className
      )}
      {...props}
    />
...
function FieldDescription({ className, ...props }: React.ComponentProps<"p">) {
  return (
    <p
      ...
      className={cn(
        "text-sm leading-normal font-normal [[data-variant=legend]+&]:-mt-1.5 text-left text-muted-foreground group-has-data-horizontal/field:text-balance",
