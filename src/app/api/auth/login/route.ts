...
export async function POST(req: NextRequest) {
  ...
  const res = NextResponse.json({ user: data.login.user })

  res.cookies.set("access_token", data.login.access_token, {
    httpOnly: true,
    secure: IS_PROD,
    sameSite: "strict",
    path: "/",
  ...
  res.cookies.set("refresh_token", data.login.refresh_token, {
    httpOnly: true,
    secure: IS_PROD,
    sameSite: "strict",
    path: "/",
