...
function getDirectViewLink(page: Page) {
  return page.getByTestId("row-direct-view")
}
...
test.describe("Projects page - employee", () => {
  ...
  test("Table is visible and contains at least one row", async ({ page }) => {
    ...
    expect(rowCount, "Expected at least one project row").toBeGreaterThan(0)
  })
  ...
  test("Row actions triggers are not present", async ({ page }) => {
    const triggers = page.getByTestId("row-actions-trigger")

    await expect(triggers).not.toBeAttached()
  })

  test("Clicking View navigates to project details page", async ({ page }) => {
    await getRows(page)
      .first()
...
test.describe("Projects page - admin", () => {
  ...
  test("Row actions popover contains View, Edit and Delete options", async ({
    page,
  }) => {
    await getRows(page)
      .first()
